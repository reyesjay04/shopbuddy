...
<div class="col-md-6 col-lg-6 col-xs-6">
	...
	<div align="right" style="margin-bottom:5px;">
		<?php display_message();?>
		<button type="button" name="add" id="add" class="btn btn-success btn-xs">Add Color</button>
	</div>
		<br>
...
<div class="col-md-6 col-lg-6 col-xs-6">
    ...
    <table id="reports_id" class="table table-striped table-bordered">
        ...
        <tbody>
            <?php show_under_categories_in_admin_color();?>
        </tbody>
    </table>
    <script>
...
<br />
		...
		<div id="user_dialog" title="Add Data">
			...
			<div class="form-group">
			<label for="product-title">Color Name</label>
			<select name="last_name" id="last_name" class="form-control" class="form-control">
				<?php
					$commandstring = "SELECT color_id , color_name FROM color  order by color_id ";
					$cmd = $sqlconnection->prepare($commandstring);
					$cmd->execute();
					$result = $cmd->fetchAll(PDO::FETCH_ASSOC);
					foreach($result as $row) {
						if($selid==$row['color_id']) {
							echo '<option value="'.$row['color_name'].'" selected="selected">'.$row['color_name'].'</option>';
						} else {
